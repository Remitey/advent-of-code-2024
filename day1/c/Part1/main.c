...
void check_before(int **update, int *row_sizes, int **rule_before,
                  int *sizes_before, int i, int *valid, int ***unvalid_update_ptr,
                  int **row_sizes_unvalid_update_ptr, int *num_rows_unvalid_ptr, 
                  int part, int *z) {
    ...
    for (int k = 0; k < sizes_before[update[i][j]]; k++) {
      for (int v = row_sizes[i] - 1; v >= 0; v--) {
        if (rule_before[update[i][j]][k] == update[i][v] && j < v) {
          *valid = 0;
          if (part == 1) {
            unvalid_update = realloc(unvalid_update, 
                                     (num_rows_unvalid + 1) * sizeof(int *));
            if (!unvalid_update) {
            ...
            row_sizes_unvalid_update = realloc(row_sizes_unvalid_update, 
                                               (num_rows_unvalid + 1) * sizeof(int));
            if (!row_sizes_unvalid_update) {
              perror("Realloc failed");
              exit(EXIT_FAILURE);
            }
...
int find_unvalid_update(int **unvalid_update, int *row_sizes_unvalid_update,
  ...
  for (int i = 0; i < num_rows_unvalid; i++) {
    valid = 1;
    for (int z = 0; z < 2; z++) {
      check_before(unvalid_update, row_sizes_unvalid_update, rule_before,
                   sizes_before, i, &valid, &unvalid_update,
                   &row_sizes_unvalid_update, &num_rows_unvalid, 2, &z);

      for (int j = 0; j < row_sizes_unvalid_update[i]; j++) {
        for (int k = 0; k < sizes_after[unvalid_update[i][j]]; k++) {
          for (int v = 0; v < row_sizes_unvalid_update[i]; v++) {
            if (rule_after[unvalid_update[i][j]][k] == unvalid_update[i][v] &&
                j > v) {
              valid = 0;
              z = 0;
              inverse_number(unvalid_update, i, j, v);
...
int find_valid_update(int **update, int *row_sizes, int num_rows,
  ...
  for (int i = 0; i < num_rows; i++) {
    ...
    check_before(update, row_sizes, rule_before, sizes_before, i, &valid,
                 &unvalid_update, &row_sizes_unvalid_update, &num_rows_unvalid, 1,
                 0);
